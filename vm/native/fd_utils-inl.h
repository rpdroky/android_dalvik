...
static const char* kPathWhitelist[] = {
  ...
  "/dev/alarm",  /* Only on Android Lollipop and below : b/31243463 */
  "/dev/log/events", /* Only on Android Kitkat and below. */
  "/dev/log/main", /* Only on Android Kitkat and below. */
  "/dev/log/radio", /* Only on Android Kitkat and below. */
  "/dev/log/system", /* Only on Android Kitkat and below. */
  "/dev/null",
  "/dev/socket/zygote",
...
class FileDescriptorInfo {
 ...
 private:
  ...
  static bool GetSocketName(const int fd, std::string* result) {
    ...
    if (TEMP_FAILURE_RETRY(getsockname(fd, &(ss.sa), &addr_len)) == -1) {
      ALOGE("Failed getsockname(%d) : %s", fd, strerror(errno));
      return false;
    }

    // Temporarily whitelist netlink sockets for kitkat.
    if (ss.sa.sa_family == AF_NETLINK) {
      (*result) = "@netlink@";
      return true;
    }

    if (ss.sa.sa_family != AF_UNIX) {
      ALOGE("Unsupported socket (fd=%d) with family %d", fd, ss.sa.sa_family);
      return false;
    }
    ...
    // std::string.
    if (ss.un.sun_path[path_len - 1] ==  '\0') {
      --path_len;
    }

    result->assign(ss.un.sun_path, path_len);
    return true;
  }
...
class FileDescriptorTable {
 public:
  ...
  static FileDescriptorTable* Create() {
    ...
    std::map<int, FileDescriptorInfo*> open_fd_map;
  ...
  // error occurred.
  bool ReopenOrDetach() {
    std::map<int, FileDescriptorInfo*>::const_iterator it;
 ...
 private:
  FileDescriptorTable(const std::map<int, FileDescriptorInfo*>& map)
      : open_fd_map_(map) {
  }
  ...
  bool RestatInternal(std::set<int>& open_fds) {
    ...
    std::map<int, FileDescriptorInfo*>::iterator it = open_fd_map_.begin();
  ...
  // Invariant: All values in this map are non-NULL.
  std::map<int, FileDescriptorInfo*> open_fd_map_;
